...
#reader(lib "beginner-reader.rkt" "deinprogramm" "sdp")((modname listen_abstrahieren_lösungen) (read-case-sensitive #f) (teachpacks ((lib "image.rkt" "teachpack" "deinprogramm" "sdp"))) (deinprogramm-settings #(#f write repeating-decimal #f #t none explicit #f ((lib "image.rkt" "teachpack" "deinprogramm" "sdp")))))
...
;(: cons-of (signature -> signature)) ; Signatur-Konstruktor
...
; Aufgabe 1: Schreiben Sie unten im angehängten File eine Funktion list-length, welche die Länge (Anzahl Elemente in der Liste) für die generische Liste list-of berechnet.
; Testen Sie diese Funktion mit einer leeren, list-of number und list-of animal.

; Länge einer Liste berechnen
(: list-length ((list-of %element) -> natural))

(check-expect (list-length empty) 0)
(check-expect (list-length (list 3 5 7)) 3)
(check-expect (list-length (list dillo1 parrot1 dillo2 parrot2)) 4)
...
; Aufgabe 2: Schreiben Sie unten im angehängten File eine Funktion live-dillos, welche aus einer Liste von Gürteltieren alle lebenden herausfiltert.

; Lebendige Gürteltiere aufsammeln
(: live-dillos ((list-of dillo) -> (list-of dillo)))

(check-expect (live-dillos (cons dillo1(cons dillo2 empty))) (cons dillo1 empty))
    ...
    (cond
      ...
      ((cons? dillos)
       (if (dillo-alive? (first dillos))
           (cons (first dillos) (live-dillos (rest dillos))) ; Konsequente
           (live-dillos (rest dillos))))))) ; Alternative
...
; Aufgabe3: Schreiben Sie unten im angehängten File eine Funktion concatenate, welche zwei Listen vom Typ list-of-number aneinanderhängt.

; Zwei Listen aneinanderhängen
(: concat (list-of-number list-of-number -> list-of-number))

; Testfall
(check-expect (concat list2 list4) (cons 5 (cons 17 (cons 2 (cons 3 (cons 5 (cons 10 empty)))))))
(check-expect (concat empty (list 2 3)) (list 2 3))
