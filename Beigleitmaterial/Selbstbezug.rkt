...
#reader(lib "beginner-reader.rkt" "deinprogramm" "sdp")((modname Selbstbezug) (read-case-sensitive #f) (teachpacks ((lib "image.rkt" "teachpack" "deinprogramm" "sdp"))) (deinprogramm-settings #(#f write repeating-decimal #f #t none explicit #f ((lib "image.rkt" "teachpack" "deinprogramm" "sdp")))))
...
(define flows-through?
  (lambda (river town)
    (cond
      ...
      ((confluence? river)
       (or
        (string=? (confluence-origin river) town)
        (flows-through? (confluence-main-stem river) town)
        (flows-through? (confluence-tributary river) town))))))
